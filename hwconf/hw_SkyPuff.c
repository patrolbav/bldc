...
void hw_init_gpio(void) {
	...
	//DRV reset pin
	palSetPadMode(BRK_RST_GPIO  , BRK_RST_PIN  , PAL_MODE_OUTPUT_OPENDRAIN);
#endif
	...
	//NRF_SWD input mode
	palSetPadMode(NRF5x_SWDIO_GPIO, NRF5x_SWDIO_PIN, PAL_MODE_INPUT_PULLUP);
	palSetPadMode(NRF5x_SWCLK_GPIO, NRF5x_SWCLK_PIN, PAL_MODE_INPUT_PULLUP);
	palSetPadMode(NRF5x_RST_GPIO  , NRF5x_RST_PIN  , PAL_MODE_OUTPUT_PUSHPULL);

	//reset NRF and DRV
	palClearPad(BRK_RST_GPIO, BRK_RST_PIN);
	palClearPad(NRF5x_RST_GPIO, NRF5x_RST_PIN);
	chThdSleep(1);
	palSetPad(BRK_RST_GPIO, BRK_RST_PIN);
	palSetPad(NRF5x_RST_GPIO, NRF5x_RST_PIN);
	...
	palSetPadMode(GPIOB, 1, PAL_STM32_MODE_ALTERNATE | PAL_MODE_ALTERNATE(GPIO_AF_TIM8));
...
float hwSkyPuff_get_temp(void) {
	float res = ADC_Value[ADC_IND_TEMP_MOS]*0.0402832 - 1.5;
    	//	res=20.0;
